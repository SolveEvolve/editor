...
// Loads the complete splat base data without touching _SplatSH. Runtime QSF
// movie frames contain only SH0 colour/opacity, so even a nominal dummy SH
// buffer must never be indexed for them.
SplatData LoadSplatDataSH0(uint idx)
{
